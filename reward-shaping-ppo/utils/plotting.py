...
def plot_learning_curves(
    env_id: str, strategies: list[str], base_dir: str = ".", grid_points: int = 100, rolling_window: int = 10
) -> None:
    """
    ...
    for strat in strategies:
        data = analyzer.load_strategy_data(strat, grid_points=grid_points, rolling_window=rolling_window)
        if data:
    ...
    for i, (strat, data) in enumerate(strat_data.items()):
        ...
        ax.fill_between(steps, stats["mean"] - stats["ci95"], stats["mean"] + stats["ci95"], color=color, alpha=0.15)

    ax.set_title(f"PPO Training Performance on {env_id} (True Unshaped Objective)", pad=15)
    ...
    for i, (strat, data) in enumerate(strat_data.items()):
        ...
        ax.plot(steps, stats["mean"], label=f"{strat.capitalize()} (Shaped)", color=color, linewidth=2)
        ax.fill_between(steps, stats["mean"] - stats["ci95"], stats["mean"] + stats["ci95"], color=color, alpha=0.15)

    ax.set_title(f"PPO Training Performance on {env_id} (Proxy Shaped Objective)", pad=15)
    ax.set_xlabel("Environment Steps")
    ...
    for i, (strat, data) in enumerate(strat_data.items()):
        ...
        ax.plot(steps, stats["mean"], label=f"{strat.capitalize()}", color=color, linewidth=2)
        ax.fill_between(steps, stats["mean"] - stats["ci95"], stats["mean"] + stats["ci95"], color=color, alpha=0.15)
...
def plot_evaluation_curves(env_id: str, strategies: list[str], base_dir: str = ".") -> None:
    """
    ...
    for i, strat in enumerate(strategies):
        ...
        for path in seed_paths:
            ...
            if os.path.exists(npz_path):
                ...
                # results has shape (n_evaluations, n_eval_episodes) -> mean over episodes
                mean_eval_rew = np.mean(data["results"], axis=1)
                strat_evals.append(mean_eval_rew)

        if len(strat_evals) > 0 and eval_steps is not None:
            has_data = True
            eval_arr = np.vstack(strat_evals)  # shape (num_seeds, n_evaluations)
            mean = np.mean(eval_arr, axis=0)
            sem = stats.sem(eval_arr, axis=0) if eval_arr.shape[0] > 1 else np.zeros_like(mean)
            ci95 = (
            ...
            color = COLOR_PALETTE[i % len(COLOR_PALETTE)]
            ax.plot(
                eval_steps, mean, label=f"{strat.capitalize()} (seeds={eval_arr.shape[0]})", color=color, linewidth=2
            )
            ax.fill_between(eval_steps, mean - ci95, mean + ci95, color=color, alpha=0.15)
    ...
    ax.set_title(f"PPO Evaluation Performance on {env_id} (Deterministic, Unshaped)", pad=15)
    ax.set_xlabel("Environment Steps")
...
def plot_training_losses(env_id: str, strategy: str, seed: int, base_dir: str = ".") -> None:
    """
    ...
    fig.suptitle(f"PPO Training Losses: {env_id} | {strategy.capitalize()} (Seed {seed})", y=0.96)
