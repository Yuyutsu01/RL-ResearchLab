...
def plot_learning_curves(
    ...
    for i, (strat, data) in enumerate(strat_data.items()):
        ...
        ax.plot(steps, stats["mean"], label=f"{strat.capitalize()} (seeds={data['raw_seeds_count']})", color=color, linewidth=2)
...
def plot_evaluation_curves(
    ...
    for i, strat in enumerate(strategies):
        ...
        if len(strat_evals) > 0 and eval_steps is not None:
            ...
            ci95 = sem * stats.t.ppf((1 + 0.95) / 2.0, eval_arr.shape[0] - 1) if eval_arr.shape[0] > 1 else np.zeros_like(mean)
            ci95 = np.nan_to_num(ci95)
            
            color = COLOR_PALETTE[i % len(COLOR_PALETTE)]
            ax.plot(eval_steps, mean, label=f"{strat.capitalize()} (seeds={eval_arr.shape[0]})", color=color, linewidth=2)
