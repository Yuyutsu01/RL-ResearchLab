...
class ExperimentAnalyzer:
    ...
    def load_strategy_data(
        ...
        for df in seed_dfs:
            ...
            rolling_orig = df["original_reward"].rolling(window=rolling_window, min_periods=1).mean().values
            rolling_shape = df["shaped_reward"].rolling(window=rolling_window, min_periods=1).mean().values
            rolling_len = df["l"].rolling(window=rolling_window, min_periods=1).mean().values
            
            # Interpolate onto common step grid
            interp_orig = np.interp(grid_steps, df["cumulative_steps"].values, rolling_orig, left=rolling_orig[0], right=rolling_orig[-1])
            interp_shape = np.interp(grid_steps, df["cumulative_steps"].values, rolling_shape, left=rolling_shape[0], right=rolling_shape[-1])
            interp_len = np.interp(grid_steps, df["cumulative_steps"].values, rolling_len, left=rolling_len[0], right=rolling_len[-1])
    ...
    def compute_summary_statistics(self, strategy: str, last_pct: float = 0.10) -> Optional[Dict[str, Any]]:
        """
    ...
    def perform_statistical_tests(self, strat1: str, strat2: str) -> Dict[str, Any]:
        ...
        t_stat, t_p = stats.ttest_ind(rewards1, rewards2, equal_var=False) if len(rewards1) > 1 and len(rewards2) > 1 else (0.0, 1.0)
        u_stat, u_p = stats.mannwhitneyu(rewards1, rewards2, alternative="two-sided") if len(rewards1) > 0 and len(rewards2) > 0 else (0.0, 1.0)
